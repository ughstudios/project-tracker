...
const en: MessageTree = {
  ...
  tools: {
    ...
    tabs: {
      "led-bandwidth": "LED link & bandwidth",
    },
    disclaimerTitle: "What this models",
    disclaimerBody:
    ...
    disclaimerBullet2:
      "Depth presets are bits per RGB channel (8 / 10 / 12). Unpacked full RGB is modeled as 3 × bpc bits per pixel (e.g. 8 bpc → 24 bpp RGB888). Packing on the wire may differ.",
    ...
    refSubtitle:
      "Floor of max pixels = usable Mbps × 1e6 ÷ (fps × 3 × bits per channel). Presets: 1G ≈ 940 Mbps, 5G ≈ 4700 Mbps.",
    colRgbDepth: "RGB depth",
    colFps: "FPS",
    colMax1g: "Max pixels @ ~940 Mbps",
    colMax5g: "Max pixels @ ~4700 Mbps",
    streamTitle: "Stream bandwidth",
    streamHelp:
      "Enter width and height to derive pixel count, or leave them blank and set total pixels. When both width and height are set, their product overrides the total pixels field.",
    labelWidth: "Width (px)",
    labelHeight: "Height (px)",
    labelPixels: "Total pixels (fallback)",
    labelFps: "Frame rate (Hz)",
    labelRgbDepth: "Depth (RGB)",
    streamPixelsNote:
      "Example: 720,000 px @ 60 Hz @ 8 bpc (RGB888, 24 bpp) ≈ 1,037 Mbps — above a ~940 Mbps 1G-class link; 10 bpc or 12 bpc needs more headroom or a faster link.",
    depthOption: {
      "8": "8 bpc — RGB888 (24 bpp / pixel)",
      "10": "10 bpc — RGB30 / 10-bit (30 bpp / pixel)",
      "12": "12 bpc — 12-bit RGB (36 bpp / pixel)",
    },
    outEffectivePixels: "Effective pixels / frame",
    outStreamMbps: "Stream bandwidth",
    outUtil1g: "Share of ~{mbps} Mbps (1G preset)",
    outUtil5g: "Share of ~{mbps} Mbps (5G preset)",
    capacityTitle: "Max pixels for a link",
    capacityHelp: "Pick a usable throughput (presets or your own number), frame rate, and RGB depth (bits per channel; 3 channels).",
...
const zh: MessageTree = {
  ...
  tools: {
    ...
    tabs: {
      "led-bandwidth": "LED 链路带宽",
    },
    disclaimerTitle: "模型说明",
    disclaimerBody:
    ...
    disclaimerBullet2:
      "色深选项为每路 R/G/B 的位数（8 / 10 / 12）。未压缩全彩按每像素 3×通道位数计算（如 8 bpc → 24 bpp，即 RGB888）。线上打包方式可能不同。",
    disclaimerBullet3: "若实测占用接近 100%，应拆分网口、降低色深或帧率，或使用厂商压缩方案。",
    refTitle: "最大像素数（参考表）",
    refSubtitle: "最大像素取下整：可用 Mbps × 1e6 ÷ (帧率 × 3 × 每通道位数)。预设：1G ≈ 940 Mbps，5G ≈ 4700 Mbps。",
    colRgbDepth: "RGB 色深",
    colFps: "帧率",
    colMax1g: "最大像素（~940 Mbps）",
    colMax5g: "最大像素（~4700 Mbps）",
    streamTitle: "码流带宽",
    streamHelp: "填写宽高以推算像素数；或留空宽高，直接填总像素。若宽高均大于 0，则使用「宽 × 高」并覆盖总像素输入。",
    labelWidth: "宽度（像素）",
    labelHeight: "高度（像素）",
    labelPixels: "总像素（备用）",
    labelFps: "帧率（Hz）",
    labelRgbDepth: "色深（RGB）",
    streamPixelsNote:
      "示例：720,000 像素 @ 60 Hz、8 bpc（RGB888，24 bpp）≈ 1,037 Mbps，高于约 940 Mbps 的类 1G；10 bpc 或 12 bpc 需更高链路或降负载。",
    depthOption: {
      "8": "8 bpc — RGB888（每像素 24 bpp）",
      "10": "10 bpc — RGB30 / 10-bit（每像素 30 bpp）",
      "12": "12 bpc — 12 位 RGB（每像素 36 bpp）",
    },
    outEffectivePixels: "每帧有效像素",
    outStreamMbps: "码流带宽",
    outUtil1g: "占 ~{mbps} Mbps（1G 预设）比例",
    outUtil5g: "占 ~{mbps} Mbps（5G 预设）比例",
    capacityTitle: "链路可承载的最大像素",
    capacityHelp: "选择可用吞吐（预设或自定义）、帧率与 RGB 色深（每通道位数，三路）。",
