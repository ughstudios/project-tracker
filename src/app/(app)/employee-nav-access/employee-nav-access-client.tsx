...
type Props = {
  ...
  /** False when the AppSettings table is missing (run prisma migrate deploy). */
  settingsDbOk: boolean;
};

export function EmployeeNavAccessClient({ initialAccess, settingsDbOk }: Props) {
  const { t } = useI18n();
  ...
  const save = async () => {
    ...
    if (!res.ok) {
      ...
      setMessage(data.error ?? t("employeeNavAccess.couldNotSave"));
      return;
    }
  ...
  return (
    <div className="space-y-4">
      {!settingsDbOk ? (
        <div
          role="alert"
          className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-950 shadow-sm"
        >
          <p className="font-semibold">{t("employeeNavAccess.migrationTitle")}</p>
          <p className="mt-2 whitespace-pre-line text-amber-900/90">
            {t("employeeNavAccess.migrationBody")}
          </p>
        </div>
      ) : null}
