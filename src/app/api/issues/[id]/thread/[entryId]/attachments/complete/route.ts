...
export async function POST(
  ...
  const prefix = `issues/${issueId}/thread/${entryId}/`;
  if (!singleSegmentAfterPrefix(pathname, prefix)) {
    return NextResponse.json({ error: "Invalid pathname." }, { status: 400 });
  }

  const ext = path.extname(fileName).toLowerCase();
  const attachment = await prisma.issueThreadAttachment.create({
    data: {
  ...
  await writeAuditLog({
    ...
    action: "UPLOAD",
    description: `${fileName} attached to thread on issue "${issue.title}".`,
  });
